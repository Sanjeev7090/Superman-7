...
const TradingDashboard = () => {
  ...
  const fetchStockData = async (ticker, tf, sourceOverride) => {
    ...
    try {
      ...
      if (src === 'groww') {
        ...
        const daysMap = {
          '1MIN':7,
          '5M':10,'10M':15,'15M':15,'30M':25,'2M':7,'3M':7,'45M':25,
          '1H':60,'2H':90,'4H':150,
          '1D':365,'1W':730,
          '1M':30,'6M':180,'1Y':730,'1MO':30,'3MO':90,'6MO':180,
        };
  ...
  // TF label → Kraken interval (minutes) + days mapping for crypto
  ...
  const fetchCryptoData = async (coinId, days, interval) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ days: String(days) });
      if (interval) params.append('interval', String(interval));
      const response = await axios.get(`${API}/crypto/chart/${coinId}?${params.toString()}`);
  ...
  const handleTimeframeChange = (tf) => {
    ...
    if (selectedStock) {
      ...
      if (selectedStock.type === 'CRYPTO') {
        // Use CRYPTO_TF_MAP for precise interval + days
        const mapping = CRYPTO_TF_MAP[tf.label] || { interval: 1440, days: 7 };
        setCryptoChartDays(mapping.days);
        fetchCryptoData(selectedStock.coin_id, mapping.days, mapping.interval);
