    ...
    <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 space-y-3" style={{ background: C.panelBg }}>
          ...
          {fiiData && fiiData.source !== 'error' && (
            <div>
              {/* Availability status */}
              {fiiData.availability && (
                <div className="rounded-lg p-2.5 mb-2" style={{ background: C.cardBg, border: `1px solid ${C.border}` }}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[9px] font-bold" style={{ color: fiiData.availability.color }}>
                      {fiiData.availability.status}
                    </span>
                    <span className="text-[8px]" style={{ color: C.textMuted }}>
                      {fiiData.availability.time_ist}
                    </span>
                  </div>
                  <div className="text-[8px]" style={{ color: C.textMuted }}>
                    {fiiData.availability.message}
                  </div>
                  {fiiData.availability.next_update && (
                    <div className="mt-1 text-[8px] font-medium" style={{ color: '#60a5fa' }}>
                      Next update: {fiiData.availability.next_update}
                    </div>
                  )}
                  {fiiData.cache_note && (
