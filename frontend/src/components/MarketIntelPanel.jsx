...
// ── EIA Wednesday countdown hook ──────────────────────────────────────────────
function useEIACountdown() {
  ...
  useEffect(() => {
    function compute() {
      ...
      const dayIST = ist.getUTCDay();   // 0=Sun … 3=Wed
      const hIST   = ist.getUTCHours();
      const mIST   = ist.getUTCMinutes();

      // EIA release: Wednesday 20:30 IST
      const EIA_H = 20, EIA_M = 30;
      const nowMinIST  = hIST * 60 + mIST;
      const eiaMinIST  = EIA_H * 60 + EIA_M;
      const diffMin    = eiaMinIST - nowMinIST; // positive = before release

      if (dayIST !== 3) {
        // Not Wednesday — show next Wednesday info
...
function CrudeSupplyCard({ brent, brentChgPct, usdinr, usdinrChgPct, geoRisk, C, isDark }) {
  ...
  // Signal 2: Brent price + day change
  const crudeSignal = (() => {
  ...
  return (
    <div className="rounded-xl overflow-hidden" data-testid="crude-supply-card"
      ...
      {/* Today's Trading Plan — always visible */}
