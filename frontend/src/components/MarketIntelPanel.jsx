...
const CRUDE_SUPPLY_TABLE = [
  { supply: 'Supply badh raha hai (Inventory ↑)', crude: 'Crude price girta hai ↓', nifty: 'Positive for Nifty ↑', bias: 'Bullish / Call Buy',    bColor: '#22c55e' },
  { supply: 'Supply kam ho raha hai (Inventory ↓)', crude: 'Crude price badhta hai ↑', nifty: 'Negative for Nifty ↓', bias: 'Bearish / Put Buy',  bColor: '#ef4444' },
  { supply: 'Supply sudden drop',                    crude: 'Crude sharp UP',            nifty: 'Strong pressure on Nifty', bias: 'Cautious / Hedge', bColor: '#f97316' },
  { supply: 'Supply sudden jump',                    crude: 'Crude sharp DOWN',           nifty: 'Relief rally in Nifty',    bias: 'Long bias',        bColor: '#22c55e' },
];
...
const INDIA_SOURCES = [
  ...
  { name: 'PPAC India',       desc: 'Official Indian data — monthly, slight delay',    url: 'https://ppac.gov.in/', tag: 'FREE', tagColor: '#3b82f6' },
  { name: 'Kpler / Vortexa',  desc: 'Real-time tanker tracking estimates (paid)',       url: 'https://kpler.com/', tag: 'PAID · ACCURATE', tagColor: '#f59e0b' },
  { name: 'Reuters / ET',     desc: 'India-specific crude news on major changes',       url: 'https://economictimes.indiatimes.com/markets/commodities', tag: 'FREE', tagColor: '#3b82f6' },
];

function CrudeSupplyCard({ brent, brentChgPct, C, isDark }) {
  const [expanded, setExpanded] = useState(false);

  // Derive live bias from brent price + day change
  const liveSignal = (() => {
    if (brent <= 0) return null;
    const chg = brentChgPct || 0;
    if (chg >= 3)
      return { label: 'SUPPLY TIGHT · CRUDE SURGING',    bias: 'Bearish / Put Buy',  color: '#ef4444', reason: `Crude +${chg.toFixed(1)}% today → supply tight → Nifty pe pressure` };
    if (chg >= 1)
      return { label: 'CRUDE RISING',                     bias: 'Cautious / Hedge',   color: '#f97316', reason: `Crude +${chg.toFixed(1)}% today → moderate upward pressure` };
    if (chg <= -3)
      return { label: 'SUPPLY EASING · CRUDE FALLING',   bias: 'Bullish / Call Buy', color: '#22c55e', reason: `Crude ${chg.toFixed(1)}% today → supply easing → Nifty ke liye supportive` };
    if (chg <= -1)
      return { label: 'CRUDE SOFTENING',                  bias: 'Mild Bullish',       color: '#4ade80', reason: `Crude ${chg.toFixed(1)}% today → slight relief` };
    return { label: 'CRUDE STABLE',                       bias: 'Neutral',            color: '#94a3b8', reason: `Crude ${chg >= 0 ? '+' : ''}${chg.toFixed(1)}% — no strong supply signal today` };
  })();
    ...
    <div className="rounded-xl overflow-hidden" data-testid="crude-supply-card"
      ...
      <div className="flex items-center justify-between px-4 py-3 cursor-pointer"
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold uppercase tracking-wide" style={{ color: C.textPrimary }}>
            Crude Oil Supply — India + Trading Logic
          </span>
          {alertZone && (
        ...
        <div className="flex items-center gap-2">
          {brent > 0 && (
            <span className="text-[10px] font-mono font-bold" style={{ color: alertZone ? '#ef4444' : '#f97316' }}>
              ${brent?.toFixed(1)}
            </span>
          )}
          <span className="text-[9px] rounded px-1 py-0.5" style={{ color: C.textMuted, background: C.panelBg }}>
      ...
      {liveSignal && (
        <div className="px-4 py-2.5 flex items-center justify-between flex-wrap gap-2"
          style={{ borderBottom: `1px solid ${C.borderSubtle}`, background: `${liveSignal.color}08` }}>
          <div>
            <div className="text-[8px] uppercase tracking-wider font-black mb-0.5" style={{ color: liveSignal.color }}>
              {liveSignal.label}
            </div>
            <div className="text-[9px]" style={{ color: C.textSecond }}>{liveSignal.reason}</div>
          </div>
          <span className="text-[9px] font-black px-2.5 py-1 rounded-full shrink-0"
            style={{ color: liveSignal.color, background: `${liveSignal.color}18`, border: `1px solid ${liveSignal.color}40` }}>
            {liveSignal.bias}
          </span>
        </div>
      )}
      ...
      {expanded && (
        <>
          {/* Supply → Crude → Nifty Logic Table */}
          <div className="px-4 py-3" style={{ borderBottom: `1px solid ${C.borderSubtle}` }}>
            <div className="text-[8px] uppercase tracking-wider font-bold mb-2" style={{ color: C.textMuted }}>
              Supply Logic Table
            </div>
            <div className="space-y-1">
              {CRUDE_SUPPLY_TABLE.map((row, i) => (
                <div key={i} className="grid gap-1 rounded-lg px-2.5 py-1.5 text-[8px]"
                  style={{
                    gridTemplateColumns: '2.2fr 1.6fr 1.6fr 1.2fr',
                    background: `${row.bColor}08`,
                    border: `1px solid ${row.bColor}20`,
                  }}>
                  <span style={{ color: C.textSecond }}>{row.supply}</span>
                  <span style={{ color: '#f97316' }}>{row.crude}</span>
                  <span style={{ color: C.textSecond }}>{row.nifty}</span>
                  <span className="font-bold" style={{ color: row.bColor }}>{row.bias}</span>
                </div>
              ))}
              {/* Column headers */}
              <div className="grid gap-1 px-2.5 text-[7px] uppercase tracking-wider mt-0.5"
                style={{ gridTemplateColumns: '2.2fr 1.6fr 1.6fr 1.2fr', color: C.textMuted }}>
                <span>Supply Situation</span><span>Crude Price</span><span>Nifty Impact</span><span>Bias</span>
              </div>
            </div>
          </div>
          ...
          <div className="px-4 py-3" style={{ borderBottom: `1px solid ${C.borderSubtle}` }}>
            <div className="text-[8px] uppercase tracking-wider font-bold mb-2" style={{ color: C.textMuted }}>
              Simple Trading Rules (Nifty ke liye)
            </div>
            <div className="space-y-1.5">
              {[
                { num: '1', rule: 'Crude Inventory Rising (Supply increase)', detail: 'Crude soft → Nifty ke liye supportive → Long / Call Buy bias', color: '#22c55e' },
                { num: '2', rule: 'Crude Inventory Falling (Supply decrease)', detail: 'Crude strong → Nifty pe pressure → Short / Put Buy ya hedge', color: '#ef4444' },
                { num: '3', rule: 'Extra Confirmation chahiye?', detail: 'Brent/WTI trend + USDINR direction + Global risk sentiment — teeno align ho to signal strong', color: '#f59e0b' },
              ].map(r => (
                <div key={r.num} className="flex items-start gap-2.5 rounded-lg px-2.5 py-1.5"
                  style={{ background: `${r.color}08`, border: `1px solid ${r.color}25` }}>
                  <span className="flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center text-[7px] font-black"
          ...
          {/* EIA Calendar + India Sources */}
          <div className="px-4 py-3" style={{ borderBottom: `1px solid ${C.borderSubtle}` }}>
            <div className="text-[8px] uppercase tracking-wider font-bold mb-2" style={{ color: C.textMuted }}>
              Data Sources — India Crude Supply Check
            </div>
            <div className="space-y-1.5">
              {INDIA_SOURCES.map(s => (
                <div key={s.name} className="flex items-center justify-between rounded-lg px-2.5 py-1.5"
                  ...
                  <span className="text-[7px] font-black px-1.5 py-0.5 rounded-full shrink-0 ml-2"
          ...
          <div className="px-4 py-3" style={{ borderBottom: `1px solid ${C.borderSubtle}` }}>
            ...
            <div className="space-y-1 text-[8.5px]" style={{ color: C.textSecond }}>
              <div className="flex items-start gap-2">
                <span style={{ color: '#f59e0b' }} className="shrink-0">★</span>
                <span><span className="font-bold" style={{ color: '#f59e0b' }}>Har Wednesday</span> — US EIA Crude Inventory data dekho (sabse important, 8:30 PM IST)</span>
              </div>
              <div className="flex items-start gap-2">
                <span style={{ color: '#3b82f6' }} className="shrink-0">●</span>
                <span>India-specific news (Kpler estimates ya ET Commodities) bhi track karo</span>
              </div>
              <div className="flex items-start gap-2">
                <span style={{ color: '#ef4444' }} className="shrink-0">▲</span>
                <span><span className="font-bold" style={{ color: '#ef4444' }}>Crude $85+ ke upar</span> + Inventory falling → Nifty mein cautious raho, fresh longs avoid karo</span>
              </div>
            </div>
          </div>
          ...
          <div className="px-4 py-2.5">
            <div className="rounded-lg px-3 py-2"
              style={{ background: isDark ? 'rgba(249,115,22,0.08)' : 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.3)' }}>
              <div className="text-[8px] font-black uppercase tracking-wider mb-1" style={{ color: '#f97316' }}>
                Short Rule (yaad rakho)
              </div>
              <div className="text-[8.5px] space-y-0.5" style={{ color: C.textSecond }}>
      ...
      >
        ...
        {data && (
          <div className="p-5 space-y-5">
            ...
            {/* ── Crude Oil Supply — India + Trading Logic ─────────────── */}
            <CrudeSupplyCard brent={data.brent} brentChgPct={data.brent_chg_pct} C={C} isDark={isDark} />
